...
Estime la densité en un point x en utilisant l'estimation de densité avec un noyau choisi.
...
"""
function densite_noyau(data::Vector{Float64}, x::Float64, h::Float64, noyau::Function)
    n = length(data)
    return sum(noyau((x - xi) / h) for xi in data) / (n * h)
end
